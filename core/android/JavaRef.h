...
#ifndef XG_F2NATIVE_JNI_JAVA_REF_H
#define XG_F2NATIVE_JNI_JAVA_REF_H
...
template <> class JavaRef<jobject> {
    public:
...
template <typename T> class ScopedJavaLocalRef : public JavaRef<T> {
    ...
    ScopedJavaLocalRef(const ScopedJavaLocalRef<T> &other) : env_(other.env_) { this->SetNewLocalRef(env_, other.obj()); }

    template <typename U> explicit ScopedJavaLocalRef(const U &other) : env_(nullptr) { this->Reset(other); }
    ...
    template <typename U> void Reset(const ScopedJavaLocalRef<U> &other) {
    ...
    template <typename U> void Reset(const U &other) {
    ...
    template <typename U> void Reset(JNIEnv *env, U obj) { env_ = this->SetNewLocalRef(env, obj); }
...
template <typename T> class ScopedJavaGlobalRef : public JavaRef<T> {
    ...
    explicit ScopedJavaGlobalRef(const ScopedJavaGlobalRef<T> &other) { this->Reset(other); }

    ScopedJavaGlobalRef(JNIEnv *env, T obj) { this->Reset(env, obj); }

    template <typename U> explicit ScopedJavaGlobalRef(const U &other) { this->Reset(other); }
    ...
    template <typename U> void Reset(JNIEnv *env, U obj) { this->SetNewGlobalRef(env, obj); }
...
#endif // XG_F2NATIVE_JNI_JAVA_REF_H
