    ...
    void Mapping(nlohmann::json &groupData, std::size_t start, std::size_t end, AbstractScale &xScale, AbstractScale &yScale, const AbstractCoord &coord) override {
        for(size_t i = start; i <= end; i++) {
            ...
            if(xVal.is_null() && yVal.is_null()) {
                ...
                item["_y"] = point.y;
                if (item.contains("original_" + fields_[0])) {
                    double xOriginal = xScale.Scale(item["original_" + fields_[0]]);
                    util::Point pointOriginal = coord.ConvertPoint(util::Point(xOriginal, y));
                    item["_original_x"] = pointOriginal.x;
                }
            }
        }
    }
...
class Color : public AttrBase {
  public:
    ...
    Color(const vector<string> &fields, const vector<string> &colors, const string &selectedColor)
    : AttrBase(fields, {"color"}), colors_(colors), selectedColor_(selectedColor){};

    Color(const string &color) : AttrBase({}, {"color"}) { colors_.push_back(color); }
    Color(const string &color, const string &selectedColor) : AttrBase({}, {"color"}), selectedColor_(selectedColor) {
    ...
    inline const string &GetSelectedColor() const {
        ...
        for(size_t i = start; i <= end; i++) {
            ...
            if(!fields_.empty() && scale::IsCategory(xScale.GetType())) {
                ...
                if (index >= 0 && index < colors_.size()){
                    ...
                    item["_selectedColor"] = colors_[index];
                } else {
                    item["_color"] = colors_[0];
                    item["_selectedColor"] = colors_[0];
                }
            } else {
                item["_color"] = colors_[0];
                item["_selectedColor"] = colors_[0];
            }
            if (selectedColor_.length() > 0) {
                item["_selectedColor"] = selectedColor_;
            }
        }
    }
