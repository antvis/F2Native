...
#include "continuous/FiveDaysLinear.h"
...
static std::unique_ptr<AbstractScale> MakeCategory(const std::string &field_,
    ...
    tracer->trace("MakeCategory: %s, return Category. ", field_.c_str());
...
static std::unique_ptr<AbstractScale> MakeLinear(const std::string &field_,
    ...
    tracer->trace("MakeLinear: %s, return Linear. ", field_.c_str());
...
static std::unique_ptr<AbstractScale> MakeScale(const std::string &field_,
    ...
    // 找出第一个不为null的值,解决第一个值为nll的情况
    nlohmann::json firstVal;
    for (size_t i = 0; i < data.size(); i++) {
        nlohmann::json dataObj = data[i];
        nlohmann::json dataVal = dataObj[field_];
        if (!dataVal.is_null()) {
            firstVal = dataVal;
            break;
        }
    ...
    if(config.contains("type")) {
        ...
        if(type == "timeCat") {
            ...
            tracer->trace("MakeScale: %s, return FiveDaysLinear. ", field_.c_str());
            return xg::make_unique<FiveDaysLinear>(field_, fieldColumn, config);
        }
    }
