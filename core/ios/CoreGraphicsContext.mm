...
static const UIColor *GetUIColorFromHexString(const std::string &hexStr,const float alpha) {
    ...
    UIColor *color =
    [UIColor colorWithRed:(rgba.r) / 255
                    green:(rgba.g) / 255
                     blue:(rgba.b) / 255
                    alpha:alpha];
    return color;
...
static UIFont *GetUIFont(const std::string &fontFmaily, float fontSize) {
    UIFont *font = [UIFont fontWithName:[NSString stringWithUTF8String:fontFmaily.c_str()] size:fontSize];
    if (!font) {
...
static NSAttributedString *GetAttributedString(CTFontRef fontRef, const UIColor *textColor, const std::string &text) {
...
static std::array<float, 2> MeasureTextSize(CTFontRef fontRef, const std::string &text) {
    //测量高度和宽度的时候颜色随便填写
    CFAttributedStringRef aString = (__bridge CFAttributedStringRef)GetAttributedString(fontRef, [UIColor whiteColor], text);
...
void CoreGraphicsContext::SetStrokeStyle(const std::string &color)  {
    ...
    CGFloat components[4] = {(strokeColor_.r) / 255, (strokeColor_.g) / 255, (strokeColor_.b) / 255, 1.0};
 ...
 */
void CoreGraphicsContext::DrawText(const std::string &text, const CanvasColor &color, float x, float y, float maxWidth) {
    size_t hash = 0;
    hash_combine(hash, fontStyle_.fontFamily, fontStyle_.fontSize);
    if (cfObjects_.find(hash) ==  cfObjects_.end()) {
        cfObjects_[hash] = std::make_unique<ScopedCFObject>((void *)(GetCTFont(fontStyle_)));
    }
    
    CTFontRef fontRef = CTFontRef(cfObjects_[hash]->object_);
    ...
    UIColor *textColor = [UIColor colorWithRed: (color.r) / 255 green: (color.g) / 255 blue: (color.b) / 255 alpha:1.0];
    NSAttributedString *attributedString = GetAttributedString(GetCTFont(fontStyle_), textColor, text);
...
void CoreGraphicsContext::BezierCurveTo(float cp1x, float cp1y, float cp2x, float cp2y, float x, float y)  {
    TraceCommand("BezierCurveTo");
    CGContextAddCurveToPoint(CGContextRef(canvasContext_), cp1x, cp1y, cp2x, cp2y, x, y);
}

void CoreGraphicsContext::Arc(float x, float y, float r, float sAngle, float eAngle, bool antiClockwise)  {
    TraceCommand("Arc");
...
void CoreGraphicsContext::DrawImage(CanvasImage *image, float dx, float dy, float width, float height)  {
    TraceCommand("DrawImage x: " + std::to_string(dx) + ", y: " + std::to_string(dy) + ", width: " +  std::to_string(width) + ", height: " + std::to_string(height));
    if (image->GetImage()) {
...
void CoreGraphicsContext::DrawLinearGradient(const canvas::CanvasFillStrokeStyle &style) {
    ...
    CGContextDrawLinearGradient(CGContextRef(canvasContext_), gradient, start, end, kCGGradientDrawsBeforeStartLocation | kCGGradientDrawsAfterEndLocation);
...
void CoreGraphicsContext::DrawRadialGradinet(const canvas::CanvasFillStrokeStyle &style) {
    ...
    CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, array, locations);
    CGContextDrawRadialGradient(CGContextRef(canvasContext_), gradient, start, startRadius, end, endRadius, kCGGradientDrawsBeforeStartLocation | kCGGradientDrawsAfterEndLocation);
    CGColorSpaceRelease(colorSpace);
    CGGradientRelease(gradient);
    CFRelease(array);
}
